...
(
  11
) NOT NULL COMMENT '父级菜单id',
  `parent_name` varchar
(
  50
) DEFAULT NULL COMMENT '父级菜单名称',
  `type` tinyint
(
  4
) NOT NULL COMMENT '菜单类型 1:目录 2：菜单 2：按钮',
  `url` varchar
(
  50
...
(
  50
) DEFAULT NULL COMMENT '授权编码',
  `color` varchar
(
  32
) DEFAULT NULL COMMENT '颜色',
  `icon` varchar
(
  32
...
INSERT INTO `t_permission` (`permission_id`, `name`, `pid`, `parent_name`, `type`, `url`, `code`, `color`, `icon`,
                            `sort`)
VALUES
...
INSERT INTO `t_role` (`role_id`, `name`, `descr`)
VALUES
  (1, '超级管理员', '最高权限');
...
(
  10
) DEFAULT NULL COMMENT '盐',
  `status` tinyint
(
  1
) NOT NULL COMMENT '状态1:启用 0：禁用',
...
INSERT INTO `t_user` (`user_id`, `user_name`, `password`, `email`, `phone`, `salt`, `status`, `create_time`,
                      `update_time`)
VALUES
(1, 'admin', '21232f297a57a5a743894a0e4a801fc3', 'admin@163.com', NULL, 'abc', 1, '2017-12-12 12:42:20',
 '2017-12-12 12:42:22');
